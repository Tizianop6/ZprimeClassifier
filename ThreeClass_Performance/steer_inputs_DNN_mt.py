...
def multiprocessing_func(mass_mt,parameters,tag):
    #parameters['exclmasseslist']=[mass_mt]
    PlotROCsMassAlternative(parameters, inputfolder=parameters['inputdir']+parameters['preprocess']+'/'+merged_str+'/'+classtag, outputfolder='output/'+parameters['preprocess']+'/DNN_'+tag, filepostfix='', plotfolder='Plots/'+parameters['preprocess']+'/DNN_'+tag+'/mass_'+str(mass_mt), use_best_model=True, usesignals=[2,4],mass=mass_mt)
...
parameters = {
    'layers':[512,512],
    'batchsize': 32768,
    'classes':{0: ['RSGluonToTT'], 1: ['ST'], 2:['WJets','DY']},#
    #'classes':{0: ['ZprimeDMToTTbarResoIncl'], 1: ['ST'], 2:['WJets','DY']},
    'name': "Kaluza-Klein gluon",
    #'name':"Z' dark matter",
    'masseslist':masseslist,
    'exclmasseslist': [],
    'regmethod': 'dropout',
    'regrate':0.50,
    'batchnorm': False,
    ...
    'siginputdir': '../Inputs_UL17_muon_signals_DM_clean/',
...
for ivars in range(len(variations)):
     ...
     if False:
         GetInputsDifferentMasses(parameters)
...
if False:
    ...
    ApplyPrepocessingAlreadyTrained(parameters, outputfolder=parameters['inputdir']+parameters['preprocess']+'/'+merged_str+'/' + classtag, filepostfix='',setid='train')
    ApplyPrepocessingAlreadyTrained(parameters, outputfolder=parameters['inputdir']+parameters['preprocess']+'/'+merged_str+'/' + classtag, filepostfix='',setid='test')
    ApplyPrepocessingAlreadyTrained(parameters, outputfolder=parameters['inputdir']+parameters['preprocess']+'/'+merged_str+'/' + classtag, filepostfix='',setid='val')
...
if newtrain:
    ##s#PlotInputs(parameters, inputfolder=inputfolder, filepostfix='', plotfolder=plotfolder+'/InputDistributions/'+merged_str+'/' + classtag)
    ...
    PredictExternalBaseline(parameters, inputfolder=parameters['inputdir']+parameters['preprocess']+'/'+merged_str+'/'+classtag, outputfolder='output/'+parameters['preprocess']+'/DNN_'+tag, filepostfix='')
    
    PredictExternalMassBaseline(parameters, inputfolder=parameters['inputdir']+parameters['preprocess']+'/'+merged_str+'/'+classtag, outputfolder='output/'+parameters['preprocess']+'/DNN_'+tag, filepostfix='',masses=masseslist)
    
    PlotPerformanceBaseline(parameters, inputfolder=parameters['inputdir']+parameters['preprocess']+'/'+merged_str+'/'+classtag, outputfolder='output/'+parameters['preprocess']+'/DNN_'+tag, filepostfix='', plotfolder='Plots/'+parameters['preprocess']+'/DNN_'+tag, use_best_model=True, usesignals=[2,4])
...
for i in range(len(masseslist)):
    p=multiprocessing.Process(target=multiprocessing_func, args=(masseslist[i],parameters,tag))
    processes.append(p)
    p.start()
    if i==5 or i==len(masseslist):
        for process in processes:
            process.join()
        processes=[]
     
    #PlotPerformanceMass(parameters, inputfolder=parameters['inputdir']+parameters['preprocess']+'/'+merged_str+'/'+classtag, outputfolder='output/'+parameters['preprocess']+'/DNN_'+tag, filepostfix='', plotfolder='Plots/'+parameters['preprocess']+'/DNN_'+tag+'/mass_'+str(masseslist[i]), use_best_model=True, usesignals=[2,4],mass=masseslist[i])
