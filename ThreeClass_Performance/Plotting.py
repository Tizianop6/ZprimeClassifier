...
def PlotMetrics(parameters, tag):
    ...
    recall_list=[]
    classification_list=[[],[],[]]
    masslistparameters=parameters["masseslist"]
    base_path='Plots/'+parameters['preprocess']+'/DNN_'+tag
    file_name="metrics.pkl"
    for mass in masslistparameters:
        ...
        if os.path.exists(file_path):
            ...
            with open(file_path, "rb") as f:
                ...
                f1_list.append(data['F1'][0])
                gmean_list.append(data['gmean'])
                pr_auc_list.append(data['pr_aucs'][0])
                precision_list.append(data['precision'][0])
                recall_list.append(data['recall'][0])
                classification_list[0].append(data["classification"][0])
                classification_list[1].append(data["classification"][1])
                classification_list[2].append(data["classification"][2])
    ...
    np_precision=np.array(precision_list,dtype=np.float32)
    np_recall=np.array(recall_list,dtype=np.float32)
    np_class0=np.array(classification_list[0],dtype=np.float32)
    np_class1=np.array(classification_list[1],dtype=np.float32)
    np_class2=np.array(classification_list[2],dtype=np.float32)
    ...
    canvases.append(ROOT.TCanvas("c10","c10",1000,700))
    graphs.append(ROOT.TGraph(len(masseslistplot),np_masses,np_recall))

    graphs[-1].SetTitle("Signal selection efficiency, "+parameters["name"] +" samples; Signal mass [GeV]; Efficiency")
    graphs[-1].SetLineColor(ROOT.kRed)
    graphs[-1].SetMarkerColor(ROOT.kRed)
    graphs[-1].SetMarkerStyle(20)
    graphs[-1].SetMarkerSize(1)
    graphs[-1].SetLineWidth(2)
    graphs[-1].Draw("ALP")
    graphs[-1].GetHistogram().SetMaximum(1.)
    graphs[-1].GetHistogram().SetMinimum(0.)
    
    canvases[-1].SaveAs(base_path+"/efficiency.pdf")

    canvases.append(ROOT.TCanvas("c11","c11",1000,700))
    graphs.append(ROOT.TGraph(len(masseslistplot),np_masses,np_class1))

    graphs[-1].SetTitle("Mistag rate in ST class; Signal mass [GeV]; Efficiency")
    ...
    canvases[-1].SaveAs(base_path+"/mistagST.pdf")


    canvases.append(ROOT.TCanvas("c12","c12",1000,700))
    graphs.append(ROOT.TGraph(len(masseslistplot),np_masses,np_class2))

    graphs[-1].SetTitle("Mistag rate in V+jets class; Signal mass [GeV]; Efficiency")
    ...
    canvases[-1].SaveAs(base_path+"/mistagV+jets.pdf")


    canvases.append(ROOT.TCanvas("c13","c13",1000,700))
    graphs.append(ROOT.TGraph(len(masseslistplot),np_masses,np_class0))

    graphs[-1].SetTitle("Correct tag class; Signal mass [GeV]; Efficiency")
    graphs[-1].SetLineColor(ROOT.kRed)
    graphs[-1].SetMarkerColor(ROOT.kRed)
    graphs[-1].SetMarkerStyle(20)
    graphs[-1].SetMarkerSize(1)
    ...
    canvases[-1].SaveAs(base_path+"/correct.pdf")


    canvases.append(ROOT.TCanvas("cmg","cmg",1000,700))
    mg1= ROOT.TMultiGraph()
    mg1.SetTitle("Signal tag efficiency and signal mistag rates, for the "+parameters["name"]+" sample; Signal mass [GeV]; Efficiency and mistag rates")
    ...
    canvases[-1].cd()
    mg1.Draw("ALP")
    canvases[-1].BuildLegend()
    canvases[-1].SaveAs(base_path+"/efficiencies_all.png")
    canvases[-1].SaveAs(base_path+"/efficiencies_all.pdf")
...
def PlotPerformanceBaseline(parameters, inputfolder, outputfolder, filepostfix, plotfolder, use_best_model=False, usesignals=[0]):
    print('Now plotting the performance')
    #gErrorIgnoreLevel = kWarning
    outputfolder_baseline="/home/pauletto/work.old/tensorflow/py3/output_originals/StandardScaler/DNN_layers_512_512__batchsize_32768__classes_3_TTbar_ST_WJets+DY__regmethod_dropout__regrate_050000__batchnorm_False__epochs_500__learningrate_000050__runonfraction_100__eqweight_False__preprocess_StandardScaler__priorSigma_100"
    ...
    # Get model and its history
    model = tf.keras.models.load_model(outputfolder_baseline+'/model.h5')
    with open(outputfolder_baseline+'/model_history.pkl', 'rb') as f:
        model_history = pickle.load(f)
...
def PlotROCsMassAlternative(parameters, inputfolder, outputfolder, filepostfix, plotfolder, use_best_model=False, usesignals=[0],mass=0):
    ...
    for k in range(len(max_value_index_labels)):
        if max_value_index_labels[k]==0:
            truesig=truesig+new_eventweights_val[k]
            classification[max_value_index_pred[k]]=classification[max_value_index_pred[k]]+new_eventweights_val[k]

    for k in range(len(classification)):
        classification[k]=classification[k]/truesig
    ...
    dict_metrics["precision"]=PRscore[0]
    dict_metrics["recall"]=PRscore[1]
    dict_metrics["classification"]=classification
