...
def FillTheta(theta_arr,masses,excl_mass=[]):
    masseslist_cp=masses.copy()
    if excl_mass:
        for i in range(len(excl_mass)):
            masseslist_cp.remove(excl_mass[i])
    nmasses=len(masseslist_cp)
    for i in range(len(theta_arr)):
        theta_arr[i]=masseslist_cp[random.randint(0,nmasses-1)]
    return theta_arr


def GetInputsDifferentMassesParametrized(parameters):
    ...
    siginputdir = parameters['siginputdir']
    bkginputdir = parameters['bkginputdir']
    ...
    if os.path.isdir('output/'+inputsubdir+systvar+'/'+prepreprocess+'/'+ classtag):
        if os.path.isfile('output/'+inputsubdir+systvar+'/'+prepreprocess+'/'+ classtag+ '/input_' + fraction + '_val.npy'):
        ...
        os.makedirs('output/'+inputsubdir+systvar+'/'+prepreprocess+'/'+ classtag)
    ...
    maxfiles_per_sample = {'TTbar': -2, 'WJets-high': 3, 'WJets-low': 3, 'WJets': -1, 'ST': -2, 'DY':-2, 'Zprime': 3, 'QCD': 3, 'RSGluonToTT': -1,'RSGluonToTT_M-6000':-1,'RSGluonToTT_M-3000':-1, 'RSGluonToTT_M-500':-1}
...
#                    thiseventweight = np.load(inputdir+systvar+inputsubdir +'Weights_' + lists_of_inputfiles[i][j])
                    ...
                    thismasslabel=np.empty(thiseventweight.shape,dtype=np.float32)
                    thistheta=np.empty(thiseventweight.shape,dtype=np.float32)
                    if (cl==3):
                        massnumberslist=re.findall(r'\d+',lists_of_inputfiles[i][j])
                        thismasslabel.fill(massnumberslist[0])
                        thistheta.fill(massnumberslist[0])
                        print('first\n massnumberlist: ', massnumberslist, '\nthismasslabel: ',thismasslabel)
                        print('cl3, file: ', lists_of_inputfiles[i][j])
                        print("thistheta:\n", thistheta)
                    else:
                        FillTheta(thistheta,masseslist)
                        thismasslabel.fill(-1.)
                        print("masseslist", masseslist)
                        thistheta=FillTheta(thistheta,masseslist,exclmasseslist)
                        print("masseslist", masseslist)
                        print('first\nthismasslabel: ',thismasslabel)
                        print('cl12, file:', lists_of_inputfiles[i][j])
                        print("thistheta:\n", thistheta)
                    thisinput=np.concatenate((thisinput,thistheta),axis=1)
                    print("thisinput\n", thisinput)
                    first = False
                else:
                    ...
                    tempthisinput=np.load(inputdir+inputsubdir+systvar+'/' + lists_of_inputfiles[i][j])
                    tempeventweight=np.load(inputdir+inputsubdir+systvar+'/'+'Weights_' + lists_of_inputfiles[i][j])
                    thiseventweight = np.concatenate((thiseventweight, tempeventweight))
                    tempmasslabel=np.empty(tempeventweight.shape,dtype=np.float32)
                    temptheta=np.empty(tempeventweight.shape,dtype=np.float32)
                    if (cl==3):
                        massnumberslist=re.findall(r'\d+',lists_of_inputfiles[i][j])
                        tempmasslabel.fill(massnumberslist[0])
                        temptheta.fill(massnumberslist[0])
                        tempthisinput=np.concatenate((tempthisinput,temptheta),axis=1)
                        thisinput=np.concatenate((thisinput,tempthisinput))
                        thismasslabel=np.concatenate((thismasslabel,tempmasslabel))
                        print('not first\n massnumberlist: ', massnumberslist, '\nthismasslabel: ',thismasslabel)
                        print('cl3, file:', lists_of_inputfiles[i][j])
                        print("temptheta:\n", temptheta)
                        
                    else:
                        tempmasslabel.fill(-1.)
                        temptheta=FillTheta(temptheta,masseslist,exclmasseslist)
                        tempthisinput=np.concatenate((tempthisinput,temptheta),axis=1)
                        thisinput=np.concatenate((thisinput,tempthisinput))
                        thismasslabel=np.concatenate((thismasslabel,tempmasslabel))
                        print('not first\nthismasslabel: ',thismasslabel)
                        print('cl012, file:', lists_of_inputfiles[i][j])
                        print("temptheta:\n", temptheta)
                    print("thisinput\n", thisinput)
        ...
        all_masslabels[cl]=thismasslabel
...
#        all_signal_eventweights[i] = thiseventweight
    ...
    if len(all_inputs) != len(classes) or len(all_labels) != len(classes) or len(all_labels) != len(all_eventweights or len(all_masslabels) != len(all_labels)):
    ...
    print('check label ', label_concatenated)
    for i in range(len(all_labels)):
      print('all labels check, len check ', all_labels[i], len(all_labels))
    ...
    masslabels_total=np.concatenate((tuple([all_masslabels[i] for i in range(len(all_masslabels))])))
    ...
    masslabels_total = masslabels_total[shuffle]
...
#        all_signal_eventweights[i] = all_signal_eventweights[i][shuffle_signal]
    ...
    print('cutoffweighted_train: ', cutoffweighted_train)
    print('cutoffweighted_test: ', cutoffweighted_test)
    print('cutoffweighted_val: ', cutoffweighted_val)
    ...
    #DA CHIEDERE QUESTA COSA
    if(takeupto_val==0):
         takeupto_val=len(eventweight_total)+1
         print("!!! TAKEUPTO VAL==0 !!!")
    ...
    masslabels_train=masslabels_total[:takeupto_train]
    ...
    masslabels_test = masslabels_total[takeupto_train:takeupto_test]
    ...
    masslabels_val = masslabels_total[takeupto_test:takeupto_val]
    ...
    print(' Test: sum val-sampleweights for each class, should be the same value for all classes')
    ...
    print('weights before ravel', eventweight_train)
    ...
    print('after ravel', eventweight_train)
#    for i in all_signal_eventweights.keys():
#        all_signal_eventweights[i] = np.asarray(all_signal_eventweights[i]).ravel()

    classtag = get_classes_tag(parameters)
#    with open(inputdir+systvar+inputsubdir+ 'variable_names.pkl', 'rb') as f:
    with open(inputdir+inputsubdir+systvar+'/'+ 'variable_names.pkl', 'rb') as f:
...
#     ### END Scaling
    ...
    output_path = 'output/'+inputsubdir+systvar+'/'+prepreprocess+'/'+classtag
    # output_path ='output/'+systvar+inputsubdir+prepreprocess+'/'+classtag
    print('Store files in ',output_path)
    variable_names.append("SigMass")
    with open(output_path+'/variable_names.pkl', 'wb') as f:
    ...
    np.save(output_path+'/masslabels_'+fraction+'_train.npy' , masslabels_train)
    np.save(output_path+'/masslabels_'+fraction+'_test.npy'  , masslabels_test)
    np.save(output_path+'/masslabels_'+fraction+'_val.npy'   , masslabels_val)
...
def GetInputs(parameters):
    ...
    #maxfiles_per_sample = {'TTbar': -1, 'WJets': -1,'Diboson': -1,'QCD': -1, 'ST': -1, 'DY': -1}
    #maxfiles_per_sample = {'TTbar': -1, 'WJets-high': -1, 'WJets-low': -1, 'WJets': -1, 'ST': -1, 'DY': -1, 'Zprime': -1, 'QCD': -1}
    maxfiles_per_sample = {'TTbar': 1, 'WJets-high': 3, 'WJets-low': 3, 'WJets': 1, 'ST': 1, 'DY': 1, 'Zprime': 3, 'QCD': 3, 'RSGluonToTT': -1}
...
def GetInputsTest(parameters):
    ...
    #maxfiles_per_sample = {'TTbar': -1, 'WJets': -1,'Diboson': -1,'QCD': -1, 'ST': -1, 'DY': -1}
    #maxfiles_per_sample = {'TTbar': -1, 'WJets-high': -1, 'WJets-low': -1, 'WJets': -1, 'ST': -1, 'DY': -1, 'Zprime': -1, 'QCD': -1}
    maxfiles_per_sample = {'TTbar': 5, 'WJets-high': 3, 'WJets-low': 3, 'WJets': -1, 'ST': 5, 'DY': 5, 'Zprime': 3, 'QCD': 3, 'RSGluonToTT': -1,'RSGluonToTT_M-6000':-1,'RSGluonToTT_M-3000':-1, 'RSGluonToTT_M-500':-1}
...
def GetInputsDifferentMasses(parameters):
    ...
    maxfiles_per_sample = {'TTbar': -1, 'WJets-high': 3, 'WJets-low': 3, 'WJets': -1, 'ST': -1, 'DY':-1, 'Zprime': 3, 'QCD': 3, 'RSGluonToTT': -1,'RSGluonToTT_M-6000':-1,'RSGluonToTT_M-3000':-1, 'RSGluonToTT_M-500':-1,'ZprimeDMToTTbarResoIncl':-1}
    # Find initial file for each class
#    inputfiles = os.listdir('input/MLInput')
#    inputfiles = os.listdir(inputdir+systvar+inputsubdir)
    inputdir=bkginputdir
...
#                    thiseventweight = np.load(inputdir+systvar+inputsubdir +'Weights_' + lists_of_inputfiles[i][j])
                    ...
                    if (cl==0):
                        ...
                        thismasslabel.fill(-1.)
                        print('first\nthismasslabel: ',thismasslabel)
                        print('c0l12, file:', lists_of_inputfiles[i][j])
                    first = False
                else:
                    # thisinput = np.concatenate((thisinput, np.load(inputdir+systvar+inputsubdir + lists_of_inputfiles[i][j])))
                    # thiseventweight = np.concatenate((thiseventweight, np.load(inputdir+systvar+inputsubdir+'Weights_' + lists_of_inputfiles[i][j])))
...
#        all_signal_eventweights[i] = all_signal_eventweights[i][shuffle_signal]
    ...
    # percentage = 0.01
    percentage = runonfraction    
    frac_train = 0.*percentage #0.6 * percentage
    frac_test  = 0 * percentage
    frac_val   = 1. * percentage #0.2 * percentage
...
def GetInputsSameW(parameters):
    ...
    #maxfiles_per_sample = {'TTbar': -1, 'WJets': -1,'Diboson': -1,'QCD': -1, 'ST': -1, 'DY': -1}
    #maxfiles_per_sample = {'TTbar': -1, 'WJets-high': -1, 'WJets-low': -1, 'WJets': -1, 'ST': -1, 'DY': -1, 'Zprime': -1, 'QCD': -1}
    maxfiles_per_sample = {'TTbar': -3, 'WJets-high': 3, 'WJets-low': 3, 'WJets': -3, 'ST': -3, 'DY': -3, 'Zprime': 3, 'QCD': 3, 'RSGluonToTT': -1}
...
#        inputfolder = parameters['inputdir']+variations[isyst]+parameters['inputsubdir']+parameters['prepreprocess']+'/'+ classtag
        ...
        print('eventweights.shape: ',eventweights_train.shape,eventweights_test.shape,eventweights_val.shape)# input_test.shape, input_val.shape)
        ...
        print('eventweights_sample.shape: after resize ',eventweights_sample.shape)#,eventweights_test.shape,eventweights_val.shape)# input_test.shape, input_val.shape)
...
def MixInputsDifferentMasses(parameters, outputfolder, variations, filepostfix):
    print("****** MixInputs ******")
    # Get parameters
    runonfraction = parameters['runonfraction']
    fraction = get_fraction(parameters)
...
#        inputfolder = parameters['inputdir']+variations[isyst]+parameters['inputsubdir']+parameters['prepreprocess']+'/'+ classtag
        ...
        print('input_.shape: ',input_train.shape, input_test.shape, input_val.shape)
        print('eventweights.shape: ',eventweights_train.shape,eventweights_test.shape,eventweights_val.shape)# input_test.shape, input_val.shape)
        print('masslabels.shape: ',masslabels_train.shape,masslabels_test.shape,masslabels_val.shape)# input_test.shape, input_val.shape)
        
        input_sample = np.concatenate((input_train,input_test,input_val), axis=0)
        labels_sample = np.concatenate((labels_train,labels_test,labels_val), axis=0)
        eventweights_sample = np.concatenate((eventweights_train,eventweights_test,eventweights_val), axis=0)
        eventweights_sample = eventweights_sample.reshape((eventweights_sample.shape[0], 1))
        masslabels_sample=np.concatenate((masslabels_train,masslabels_test,masslabels_val),axis=0)
        
        print('eventweights_sample.shape: after resize ',eventweights_sample.shape)#,eventweights_test.shape,eventweights_val.shape)# input_test.shape, input_val.shape)
        print('masslabels_sample.shape',masslabels_sample.shape)
        #FixMe sample weigtht containts weight to equalize the samples and calculated per systematic variation
        #for merged samples it should be recalculated. However using equally weighted samples does not give advantage, 
        #thus this variable is not going to be used in the nearest future and left it as it's now
        sample_weights_sample = np.concatenate((sample_weights_train,sample_weights_test,sample_weights_val), axis=0) 
...
#        input_array = np.concatenate((input_sample,labels_sample,eventweights_sample), axis=1) #array with all backgrounds at one place
        input_array = np.concatenate((input_sample,labels_sample,eventweights_sample,sample_weights_sample,masslabels_sample), axis=1) #array with all backgrounds at one place
        if input_array_all[0,0]!=1:
            print("input_array_all[0,0]!=1")
            tmp_input_array = np.append(input_array_all, input_array,axis=0)
            input_array_all.resize(tmp_input_array.shape[0],tmp_input_array.shape[1])
            input_array_all = tmp_input_array.copy()
        else:
...
def SplitInputsTest(parameters, outputfolder, filepostfix):
    print("****** SplitInputs ******")
    classtag = get_classes_tag(parameters)
    runonfraction = parameters['runonfraction']
    fraction = get_fraction(parameters)
...
#    inputfolder = parameters['inputdir']+'NOMINAL'+parameters['inputsubdir']+parameters['prepreprocess']+'/'+ classtag
    ...
    train_set = np.concatenate((train1_set, train2_set, train3_set,test_set,val_set), axis=0)
    ...
    f_train.close()
    f_test = gzip.GzipFile(outputfolder+'/input_'+fraction+'_test_set_raw.npy.gz', "w")
    np.save(file=f_test, arr=train_set[:,0:input_train_shape1])
    #np.save(file=f, arr=test_set)
    f_test.close()
    f_val = gzip.GzipFile(outputfolder+'/input_'+fraction+'_val_set_raw.npy.gz', "w")
    np.save(file=f_val, arr=train_set[:,0:input_train_shape1])
#    np.save(file=f, arr=val_set)
    f_val.close()
    ...
    labels_train_shape_1 = len(parameters['classes'])
    np.save(outputfolder+'/labels_'+fraction+'_train.npy', train_set[:,input_train_shape1:input_train_shape1+labels_train_shape_1])
    np.save(outputfolder+'/labels_'+fraction+'_test.npy', train_set[:,input_train_shape1:input_train_shape1+labels_train_shape_1])
    np.save(outputfolder+'/labels_'+fraction+'_val.npy', train_set[:,input_train_shape1:input_train_shape1+labels_train_shape_1])
    print('Labels stored!')
    np.save(outputfolder+'/eventweights_'+fraction+'_train.npy', train_set[:,-2])
    np.save(outputfolder+'/eventweights_'+fraction+'_val.npy', train_set[:,-2])
    np.save(outputfolder+'/eventweights_'+fraction+'_test.npy', train_set[:,-2])
    print('Weights test stored!')
    np.save(outputfolder+'/sample_weights_'+fraction+'_train.npy', train_set[:,-1])
    np.save(outputfolder+'/sample_weights_'+fraction+'_val.npy', train_set[:,-1])
    np.save(outputfolder+'/sample_weights_'+fraction+'_test.npy', train_set[:,-1])
    print('Sample Weights test stored!')
...
def SplitInputsDifferentMasses(parameters, outputfolder, filepostfix):
    print("****** SplitInputs ******")
    classtag = get_classes_tag(parameters)
    runonfraction = parameters['runonfraction']
    fraction = get_fraction(parameters)
...
#    inputfolder = parameters['inputdir']+'NOMINAL'+parameters['inputsubdir']+parameters['prepreprocess']+'/'+ classtag
    ...
    all_set=np.concatenate((train1_set, train2_set, train3_set,test_set,val_set), axis=0)
...
#    np.save(file=f, arr=val_set)
    ...
    np.save(outputfolder+'/labels_'+fraction+'_val.npy', val_set[:,input_train_shape1:input_train_shape1+labels_train_shape_1])
    print('Labels stored!')
    np.save(outputfolder+'/eventweights_'+fraction+'_train.npy', train_set[:,-3])
    np.save(outputfolder+'/eventweights_'+fraction+'_val.npy', val_set[:,-3])
    np.save(outputfolder+'/eventweights_'+fraction+'_test.npy', test_set[:,-3])
    print('Weights test stored!')
    np.save(outputfolder+'/sample_weights_'+fraction+'_train.npy', train_set[:,-2])
    np.save(outputfolder+'/sample_weights_'+fraction+'_val.npy', val_set[:,-2])
    np.save(outputfolder+'/sample_weights_'+fraction+'_test.npy', test_set[:,-2])
    print('Sample Weights test stored!')
    np.save(outputfolder+'/masslabels_'+fraction+'_train.npy', train_set[:,-1])
    np.save(outputfolder+'/masslabels_'+fraction+'_val.npy', val_set[:,-1])
    np.save(outputfolder+'/masslabels_'+fraction+'_test.npy', test_set[:,-1])
    print('MassLabels stored!')
...
'''
def SplitInputsDifferentMassesParametrized(parameters, outputfolder, filepostfix):
    print("****** SplitInputs ******")
    classtag = get_classes_tag(parameters)
    runonfraction = parameters['runonfraction']
    fraction = get_fraction(parameters)
...
def FitPrepocessingParametrized(parameters, outputfolder, filepostfix):
    print("****** FitPrepocessing ******")
    classtag = get_classes_tag(parameters)
    runonfraction = parameters['runonfraction']
    fraction = get_fraction(parameters)
    tag = dict_to_str(parameters)
    with open(outputfolder+ '/variable_names.pkl', 'rb') as f:
        variable_names = pickle.load(f)
    input_train_shape1 = len(variable_names)
    ...
    with open(outputfolder+'/NormInfo.txt', 'w') as f:
        for i in range(np.mean(input_train_set, axis=0).shape[0]): #valid only for StandardScaler, placeholder for the rest
            ...
            if(parameters['preprocess'] == 'StandardScaler'):
                ...
                #if i==(np.mean(input_train_set, axis=0).shape[0]-1):
                #    mean=0.
                #    scale=1.
                line = var + ' StandardScaler ' + str(mean) + ' ' + str(scale) + '\n'
            elif(parameters['preprocess'] == 'MinMaxScaler'):
...
def ApplyPrepocessing(parameters, outputfolder, filepostfix, setid):
    ...
    if(parameters['preprocess'] == 'StandardScaler'):
        ...
        Xmean, Xstd = scaler_data_[0], scaler_data_[1]
        input_train_set = (input_train_set - Xmean) / (Xstd)
    elif(parameters['preprocess'] == 'MinMaxScaler'):
        scaler_data_ = np.load(outputfolder+'/NormInfo.npy')
...
def ApplyPrepocessingAlreadyTrained(parameters, outputfolder, filepostfix, setid):
    print("****** ApplyPrepocessing ******")
    classtag = get_classes_tag(parameters)
    runonfraction = parameters['runonfraction']
    fraction = get_fraction(parameters)
    ...
    #input_train_set = train_set[:,0:input_train_shape1]
    input_train_set = train_set
    outputfolder_baseline="/home/pauletto/work.old/tensorflow/py3/output_originals/StandardScaler/Merged__NOMINAL/classes_3_TTbar_ST_WJets+DY"
    if(parameters['preprocess'] == 'StandardScaler'):
        scaler_data_ = np.load(outputfolder_baseline+'/NormInfo.npy')
        Xmean, Xstd = scaler_data_[0], scaler_data_[1]
        input_train_set = (input_train_set - Xmean) / (Xstd)
    elif(parameters['preprocess'] == 'MinMaxScaler'):
        scaler_data_ = np.load(outputfolder_baseline+'/NormInfo.npy')
...
def ApplyPrepocessingTest(parameters, outputfolder, filepostfix, setid,outputfolder_orig):
    print("****** ApplyPrepocessing ******")
    classtag = get_classes_tag(parameters)
    runonfraction = parameters['runonfraction']
    fraction = get_fraction(parameters)
    ...
    if(parameters['preprocess'] == 'StandardScaler'):
        scaler_data_ = np.load(outputfolder_orig+'/NormInfo.npy')
        Xmean, Xstd = scaler_data_[0], scaler_data_[1]
        input_train_set = (input_train_set - Xmean) / (Xstd)
    elif(parameters['preprocess'] == 'MinMaxScaler'):
        scaler_data_ = np.load(outputfolder_orig+'/NormInfo.npy')
