...
def PredictExternal(parameters, inputfolder, outputfolder, filepostfix):
    ...
    model_best = tf.keras.models.load_model(outputfolder+'/model_best.h5')
...
def PredictExternalBaseline(parameters, inputfolder, outputfolder, filepostfix):
    print('Making predictions now')
    tag = dict_to_str(parameters)
    classtag = get_classes_tag(parameters)
    fraction = get_fraction(parameters)
    ...
    model_best = tf.keras.models.load_model(outputfolder_baseline+'/model_best.h5')
    input_train, input_test, input_val, labels_train, labels_test, labels_val, sample_weights_train, sample_weights_test, sample_weights_val, eventweights_train, eventweights_test, eventweights_val, signals, signal_eventweights, signal_normweights = load_data(parameters, inputfolder=inputfolder, filepostfix=filepostfix)
...
def PredictExternalExclMass(parameters, inputfolder, outputfolder, filepostfix):
    print('Making predictions now')
    tag = dict_to_str(parameters)
    classtag = get_classes_tag(parameters)
    fraction = get_fraction(parameters)
...
def PredictExternalMass(parameters, inputfolder, outputfolder, filepostfix,masses):
    print('Making predictions now')
    tag = dict_to_str(parameters)
    classtag = get_classes_tag(parameters)
    fraction = get_fraction(parameters)
    ...
    model = tf.keras.models.load_model(outputfolder+'/model.h5')
    model_best = tf.keras.models.load_model(outputfolder+'/model_best.h5')
    input_train, input_test, input_val, labels_train, labels_test, labels_val, sample_weights_train, sample_weights_test, sample_weights_val, eventweights_train, eventweights_test, eventweights_val, signals, signal_eventweights, signal_normweights, masslabels_train, masslabels_test, masslabels_val = load_data_differentmasses(parameters, inputfolder=inputfolder, filepostfix=filepostfix)
    outputfolder_orig=outputfolder

#    signal_identifiers = ['RSGluon_All', 'RSGluon_M1000', 'RSGluon_M2000', 'RSGluon_M3000', 'RSGluon_M4000', 'RSGluon_M5000', 'RSGluon_M6000']
    for j in range(len(masses)):
        outputfolder=outputfolder_orig+'/m_'+str(masses[j])
    ...
    # print("input_train[2]",input_train[2])
    # print ("Keras pred_train[2] =",pred_train[2])
    print('len(masslabels_val)', masslabels_val)
    print('len(input_val[:,0])', len(input_val[:,0]))
    idxtodel=[]
    for j in range(len(masses)):
        outputfolder=outputfolder_orig+'/m_'+str(masses[j])
        np.save(outputfolder+'/prediction_train.npy'  , pred_train)
    ...
    for j in range(len(masses)):
        ...
        for i in range(len(masslabels_val)):
            ...
            if (masslabels_val[i]!=masses[j] and masslabels_val[i]!=-1):
        ...
        print('after deleting new_input_val.shape ', new_input_val.shape)

        pred_val = model.predict(new_input_val)

        np.save(outputfolder+'/prediction_val.npy'  , pred_val)
        for cl in range(len(parameters['classes'])):
            print('predicting for val set, class ' + str(cl))
            tmp = pred_val[new_labels_val[:,cl] == 1]
            np.save(outputfolder+'/prediction_val_class'+str(cl)+'.npy'  , tmp)
    ...
    for j in range(len(masses)):
        ...
        pred_val = model_best.predict(new_input_val)
...
def PredictExternalMassBaseline(parameters, inputfolder, outputfolder, filepostfix,masses):
    print('Making predictions now')
    tag = dict_to_str(parameters)
    classtag = get_classes_tag(parameters)
    fraction = get_fraction(parameters)
...
def PredictExternalAlreadyTrained(parameters, inputfolder, outputfolder, filepostfix,outputfolder_trained):
    print('Making predictions now')
    tag = dict_to_str(parameters)
    classtag = get_classes_tag(parameters)
    fraction = get_fraction(parameters)
    if not os.path.isdir(outputfolder): os.makedirs(outputfolder)
    # Get inputs
    model = tf.keras.models.load_model(outputfolder_trained+'/model.h5')
    model_best = tf.keras.models.load_model(outputfolder_trained+'/model_best.h5')
    input_train, input_test, input_val, labels_train, labels_test, labels_val, sample_weights_train, sample_weights_test, sample_weights_val, eventweights_train, eventweights_test, eventweights_val, signals, signal_eventweights, signal_normweights = load_data(parameters, inputfolder=inputfolder, filepostfix=filepostfix)
