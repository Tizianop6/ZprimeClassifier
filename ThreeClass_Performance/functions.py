...
def dict_to_str(parameters):
    ...
    tag = tag + "exclM_"+"_".join(str(x) for x in parameters['exclmasseslist'])
...
def load_data_excl_mass(parameters, inputfolder, filepostfix,exclmasslist):
    ...
    masslabels_train = np.load(inputfolder+'/masslabels_'+fraction+'_train'+filepostfix+'.npy').astype(np.float32)
    masslabels_test = np.load(inputfolder+'/masslabels_'+fraction+'_test'+filepostfix+'.npy').astype(np.float32)
    masslabels_val = np.load(inputfolder+'/masslabels_'+fraction+'_val'+filepostfix+'.npy').astype(np.float32)

    print("load data excl_masses: ", exclmasslist)
    print("train data before cleaning: \n",input_train.shape,"\n",input_train)


    for i in range(len(exclmasslist)):
        idxtodel=[]
        for j in range(len(masslabels_train)):
            if (masslabels_train[j]==exclmasslist[i]):
                #print('idx: ',i, "masslabels_val[i] ", masslabels_val[i])
                idxtodel.append(j)
        input_train=np.delete(input_train,idxtodel,axis=0)
        labels_train=np.delete(labels_train,idxtodel,axis=0)
        sample_weights_train=np.delete(sample_weights_train,idxtodel,axis=0)
        eventweights_train=np.delete(eventweights_train,idxtodel,axis=0)
        masslabels_train=np.delete(masslabels_train,idxtodel,axis=0)
        
        idxtodel=[]
        for j in range(len(masslabels_val)):
            if (masslabels_val[j]==exclmasslist[i]):
                #print('idx: ',i, "masslabels_val[i] ", masslabels_val[i])
                idxtodel.append(j)
        input_val=np.delete(input_val,idxtodel,axis=0)
        labels_val=np.delete(labels_val,idxtodel,axis=0)
        sample_weights_val=np.delete(sample_weights_val,idxtodel,axis=0)
        eventweights_val=np.delete(eventweights_val,idxtodel,axis=0)
        masslabels_val=np.delete(masslabels_val,idxtodel,axis=0)        
        
        idxtodel=[]
        for j in range(len(masslabels_test)):
            if (masslabels_test[j]==exclmasslist[i]):
                #print('idx: ',i, "masslabels_val[i] ", masslabels_val[i])
                idxtodel.append(j)
        input_test=np.delete(input_test,idxtodel,axis=0)
        labels_test=np.delete(labels_test,idxtodel,axis=0)
        sample_weights_test=np.delete(sample_weights_test,idxtodel,axis=0)
        eventweights_test=np.delete(eventweights_test,idxtodel,axis=0)
        masslabels_test=np.delete(masslabels_test,idxtodel,axis=0)        
...
def load_data_differentmasses(parameters, inputfolder, filepostfix):
...
#        sum_signal_eventweights = signal_eventweights[i].sum()
#        signal_normweights[i] = np.array([1./sum_signal_eventweights for j in range(signal_eventweights[i].shape[0])])
    return input_train, input_test, input_val, labels_train, labels_test, labels_val, sample_weights_train, sample_weights_test, sample_weights_val, eventweights_train, eventweights_test, eventweights_val, signals, signal_eventweights, signal_normweights, masslabels_train, masslabels_test, masslabels_val
...
def roc_curve_own(y_true, y_score, pos_label=None, sample_weight=None, drop_intermediate=False):
...
#    print("HEY, in <plot_rocs> pred_val: ",pred_val[0:2,:])
 #   print("HEY, in <plot_rocs> labels_val: ",labels_val[0:2,:])
    ...
    for i in range(len(FalsePositiveRates)):
        ...
        plt.plot(FalsePositiveRates[i], TruePositiveRates[i], label=classtitles[i] + ', AUC: '+str(round(aucs[i],3)), color=colorstr[i])
    ...
    for i in range(len(FalsePositiveRates)):
        #plt.semilogy(TruePositiveRates[i], FalsePositiveRates[i], label=classtitles[i] + ', AUC: '+str(round(aucs[i],3)), color=colorstr[i])
        plt.plot( Thresholds[i],FalsePositiveRates[i], label=classtitles[i] + ', AUC: '+str(round(aucs[i],3)), color=colorstr[i])
    plt.plot([0, 0], [1, 1], color='k', linestyle='-', linewidth=1)
    plt.legend(loc='upper left')
    #plt.ylim([0.0001, 1.05])
    plt.ylim([0.0, 1.0])
    ...
    plt.ylabel('Background efficiency')
    title = 'ROC_val_eqweight_thresholds'
    if use_best_model: title += '_best'
    title += '.pdf'
    fig.savefig(plotfolder+'/'+title)
    plt.close()
    ...
    for i in range(len(FalsePositiveRates)):
        #plt.semilogy(TruePositiveRates[i], FalsePositiveRates[i], label=classtitles[i] + ', AUC: '+str(round(aucs[i],3)), color=colorstr[i])
        plt.plot(Thresholds[i], TruePositiveRates[i], label=classtitles[i] + ', AUC: '+str(round(aucs[i],3)), color=colorstr[i])
    plt.plot([0, 0], [1, 1], color='k', linestyle='-', linewidth=1)
    plt.legend(loc='upper left')
    #plt.ylim([0.0001, 1.05])
    plt.ylim([0.0, 1.0])
    ...
    plt.ylabel('Selection efficiency')
    plt.xlabel('Thresholds')
    title = 'ROC_val_eqweight_thresholds_2'
    if use_best_model: title += '_best'
    title += '.pdf'
    fig.savefig(plotfolder+'/'+title)
    plt.close()
    ...
    #roc_sklearn=roc_auc_score(labels_val,pred_val, average='weighted',sample_weight=eventweights_val,multi_class='ovr')
    for i in range(len(FalsePositiveRates)):
        AUCS_dict[classtitles[i]]=aucs[i]
        #plt.semilogy(TruePositiveRates[i], FalsePositiveRates[i], label=classtitles[i] + ', AUC: '+str(round(aucs[i],3)), color=colorstr[i])
    #AUCS={}

    #print("rok_sklearn\n",roc_sklearn,"\n AUCS old way:\n ", AUCS_dict)
    with open(plotfolder+'/AUCS.pkl', 'wb') as f:
...
#        auc_signals_lum = {}
        ...
        if do_sig:
            for key in list(pred_signals.keys()):
...
def plot_PR_curves(parameters, plotfolder, pred_val, labels_val, sample_weights_val, eventweights_val, pred_signals=None, eventweight_signals=None, usesignals=[0], use_best_model=False):
...
#    print("HEY, in <plot_rocs> pred_val: ",pred_val[0:2,:])
 #   print("HEY, in <plot_rocs> labels_val: ",labels_val[0:2,:])
    #precision, recall, thresholds = precision_recall_curve(labels_val[], parameters=parameters, pred_val=pred_val, labels_val=labels_val, weights_val=sample_weights_val)
    # print 'eqweight: ', sample_weights_val[10:15]
    ...
    for k in range(len(labels_val[0])):
        precision, recall, thresholds = precision_recall_curve(labels_val[:,k],pred_val[:,k],sample_weight=sample_weights_val)
        newrecall=[]
        newprecision=[]
        for u in range(len(recall)-1):
            if not (recall[u]==recall[u+1]):
                newrecall.append(recall[u])
                newprecision.append(precision[u])
        auc_precision_recall = np.trapz(np.flipud(np.array(newprecision)),x=np.flipud(np.array(newrecall)))
        aucs_list.append(auc_precision_recall)
        #plt.semilogy(TruePositiveRates[i], FalsePositiveRates[i], label=classtitles[i] + ', AUC: '+str(round(aucs[i],3)), color=colorstr[i])
        plt.plot(recall,precision, label=classtitles[k] + ', AUC: '+str(round(auc_precision_recall,3)), color=colorstr[k])
    plt.plot([0, 0], [1, 1], color='k', linestyle='-', linewidth=1)
    plt.legend(loc='upper left')
    #plt.ylim([0.0001, 1.05])
    plt.ylim([0.0, 1.0])
...
def plot_accuracy(parameters, plotfolder, model_history):
    ...
    fig = plt.figure()
    plt.grid()
    x = list(range(len(model_history['categorical_accuracy'])+1))
    x = x[1:]
...
def plot_weight_updates(parameters, model, input_val):
    ...
    for weightfile in weightfiles:
        model = tf.keras.models.load_model('output/'+tag+'/'+weightfile)
        weights_thismodel = []
        updates_thismodel = []
...
def plot_outputs_2d(parameters, plotfolder, pred_vals, lumiweights_vals, use_best_model=False):
    ...
    gROOT.SetBatch(True);
    for i in list(classes.keys()):
        for j in list(classes.keys()):
            if j <= i: continue
            idx = 0
            hists = []
            for trueclass in list(classes.keys()):
...
#            print "yerrs_norm[i]",yerrs_norm[i]
        ...
        classtitle_to_use = ''
        for i in range(labels_train.shape[1]):
            plt.hist(pred_trains[i][cl], weights=weights_trains[i][cl], bins=nbins, histtype='step', label='Training sample, ' + classtitles[i], color=colorstr[i])
