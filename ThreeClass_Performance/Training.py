...
from tensorflow.keras.callbacks import History, ModelCheckpoint, ReduceLROnPlateau, EarlyStopping, TensorBoard #for keras+theano
...
def TrainNetwork(parameters, inputfolder, outputfolder):
    ...
    try:
        #model = tf.keras.models.load_model('output/'+tag+'/model.h5')
    ...
    # Train the network
    opt = tf.keras.optimizers.Adam(lr=learningrate, beta_1=0.9, beta_2=0.999, epsilon=1e-6, decay=0.0, amsgrad=False)
...
def TrainNetworkExclMass(parameters, inputfolder, outputfolder):
...
def TrainForMoreEpochs(parameters, nepochs):
    ...
    try:
        model = tf.keras.models.load_model('output/'+new_tag+'/model.h5')
    ...
    with open('input/'+classtag+'/sample_weights_'+fraction+'_train.pkl', 'rb') as f:
        sample_weights_train = pickle.load(f)
    with open('input/'+classtag+'/eventweights_'+fraction+'_train.pkl', 'rb') as f:
        eventweights_train = pickle.load(f)
    with open('input/'+classtag+'/sample_weights_'+fraction+'_test.pkl', 'rb') as f:
        sample_weights_test = pickle.load(f)
    with open('input/'+classtag+'/eventweights_'+fraction+'_test.pkl', 'rb') as f:
        eventweights_test = pickle.load(f)
    with open('input/'+classtag+'/sample_weights_'+fraction+'_val.pkl', 'rb') as f:
        sample_weights_val = pickle.load(f)
    with open('input/'+classtag+'/eventweights_'+fraction+'_val.pkl', 'rb') as f:
    ...
    # Train the network
    opt = tf.keras.optimizers.Adam(lr=learningrate, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
    mymetrics = [metrics.categorical_accuracy]
...
#        print("weights.shape = ",weights.shape)
        ...
        opt = tf.train.AdamOptimizer(lr=learningrate, beta1=0.9, beta2=0.999, epsilon=1e-8, use_locking=False) #TEST
        #opt = tf.keras.optimizers.Adam(lr=learningrate, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
...
#         #with open('output/BNN_'+tag+'/model_history.pkl', 'wb') as f:
#         #    pickle.dump(model.history.history, f)
...
#     #With dropout
    ...
    with open('output/DNN_'+tag+'/model_history.pkl', 'wb') as f:
        pickle.dump(model.history.history, f)
