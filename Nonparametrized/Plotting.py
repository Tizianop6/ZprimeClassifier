...
def PlotMetrics(parameters, tag):
    ...
    for mass in range(500, 6001, 500):
        ...
        if os.path.exists(file_path):
            ...
            with open(file_path, "rb") as f:
                ...
                precision_list.append(data['precision'][3])
                recall_list.append(data['recall'][3])
    ...
    canvases.append(ROOT.TCanvas("c21","c21",1000,700))
    graphs.append(ROOT.TGraph(len(masseslistplot),np_masses,np_precision))
    
    graphs[-1].SetTitle("Precision, signal class; Signal mass [GeV]; F1 score")
    graphs[-1].SetLineColor(ROOT.kRed)
    graphs[-1].SetMarkerColor(ROOT.kRed)
    graphs[-1].SetMarkerStyle(20)
    graphs[-1].SetMarkerSize(1)
    ...
    canvases[-1].SaveAs(base_path+"/precision.pdf")

    canvases.append(ROOT.TCanvas("c22","c22",1000,700))
    graphs.append(ROOT.TGraph(len(masseslistplot),np_masses,np_recall))
    
    graphs[-1].SetTitle("Recall, signal class; Signal mass [GeV]; F1 score")
    ...
    canvases[-1].SaveAs(base_path+"/recall.pdf")


    canvases.append(ROOT.TCanvas("cmg","cmg",1000,700))
    mg1= ROOT.TMultiGraph()
    mg1.SetTitle("Precision, recall and F1 score for the "+parameters["name"]+" class; Signal mass [GeV]; Scores")
    ...
    canvases[-1].SaveAs(base_path+"/p-r-f1.png")
    canvases[-1].SaveAs(base_path+"/p-r-f1.pdf")
    ...
    canvases.append(ROOT.TCanvas("c41","c41",1000,700))
    canvases[-1].cd()
    mg1.Add(graphs[-1])
    mg1.Draw("ALP")
    canvases[-1].BuildLegend()
    canvases[-1].SaveAs(base_path+"/p-r-f1-prauc.png")
    canvases[-1].SaveAs(base_path+"/p-r-f1-prauc.pdf")
